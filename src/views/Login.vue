<template>
	<div style="height: 100%;width: 100%;background-image:@/url(../assets/background.jpg);">
		...
		<el-form :model="loginForm" :rules="fieldRules" ref="loginForm" label-position="left" label-width="0px" class="demo-ruleForm login-container">
		  ...
		  <el-form-item style="width:100%;">
		    <el-button type="primary" style="width:48%;" @click.native.prevent="login" :loading="loading">登 录</el-button>
				<el-button type="primary" plain style="width:48%;" @click.native.prevent="viewLogin">演示用户</el-button>
		  </el-form-item>
		</el-form>
	</div>
</template>
  ...
  methods: {
    login() {
      ...
      this.$api.login.login(userInfo).then((res) => {
		...
		viewLogin(){
			this.loading = true
			let userInfo = {account:'view', password:'view'}
			this.$api.login.login(userInfo).then((res) => {
			    if(res.msg != null) {
			      this.$message({
			        message: res.msg,
